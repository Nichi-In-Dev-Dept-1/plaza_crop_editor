...
abstract class BaseCroppableImageController extends ChangeNotifier {
  ...
  /// Sets the value for [_resetData], i.e. the value that the controller will
  /// default to when the "Reset" button is pressed.
  ...
  /// Sets the [_initialData], i.e. the data that was passed to this controller
  /// when it was created.
  ...
  Future<CropImageResult> crop(
      {ImageProvider<Object>? imageProvider, CroppableImageData? data}) async {
    final image = await obtainImage(imageProvider?? this.imageProvider);
    final result = await cropImage(image, data??this.data);
